...
func printUserData(jsonData string, age int) string {
    ...
    if res.Age > age {
        ...
        output = fmt.Sprintf("User %s %s must be over %d before we print their details", res.FirstName, res.LastName, age)
    }
...
func main() {
    var age int = 24
    str := `{"first_name": "Matthew", "last_name": "Setter", "age": 29, "languages": ["php", "javascript", "go"]}`
